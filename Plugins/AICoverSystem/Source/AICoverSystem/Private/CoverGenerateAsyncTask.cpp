...
FCoverGenerateAsyncTask::FCoverGenerateAsyncTask(UWorld* World, FCoverSystemProxy* InProxy, ARecastNavMesh* NavMesh, const FCoverBuildParams& Params, const FVector& InOrigin, const TArray<FCoverData>* AdditionalData)
{
	check(World);
	check(InProxy);
	check(NavMesh);
...
void FCoverGenerateAsyncTask::DoWork()
{
	...
	*/
	...
	if (Builder.Build(Memory, WorldPtr.Get(), NavMeshPtr.Get(), BuildParams, AdditionalCoverData))
	{
