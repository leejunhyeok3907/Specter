...
bool FCoverSystemProxy::GenerateCovers(bool bAsync, bool bRegenerate, UWorld* World, const FCoverBuildParams& Params, const FVector& Origin, const TArray<FCoverData>* AdditionalData)
{
	check(IsInGameThread());
	...
	// Run build task
	FAsyncTask<FCoverGenerateAsyncTask>* Task = new FAsyncTask<FCoverGenerateAsyncTask>(World, this, NavMesh, Params, Origin, AdditionalData);
	check(Task);
